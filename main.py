...
def get_args_parser():
    ...
    # Model parameters
    parser.add_argument('--model', default='deit_tiny_patch16_224_KAN', type=str, metavar='MODEL',
    ...
    parser.add_argument('--data-path', default='/scratch/sg7729/KAN/Vision-KAN/Datasets', type=str,
                        help='dataset path')
    parser.add_argument('--data-set', default='CIFAR', choices=['CIFAR', 'IMNET', 'INAT', 'INAT19','TinyIMNET','CIFAR10'],
                        type=str, help='CIFAR dataset path')
    parser.add_argument('--inat-category', default='name',
                        choices=['kingdom', 'phylum', 'class', 'order', 'supercategory', 'family', 'genus', 'name'],
                        type=str, help='semantic granularity')

    parser.add_argument('--output_dir', default='./moe10-results',
...
#             if len(parts) == 3:
#                 # 只有当每项数据确切包含三个部分时才添加
#                 self.data.append(tuple(map(float, parts)))
            # else:
            #     raise ValueError(f"Expected 3 values per entry, got {len(parts)}: {s}")

    def __len__(self):
        return len(self.data)

    def __getitem__(self, idx):
        # 根据索引获取数据，并将其转换为所需的tensor格式
...
def main(args):
   ...
   # dataset_train = CustomDataset(ds['train']['text'])
